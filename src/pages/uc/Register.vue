                ...
                <Form v-if="allowRegister" ref="formInline" :model="formInline" :rules="ruleInline" inline>
                    ...
                    <FormItem prop="country">
                        <Select v-model="formInline.country" :placeholder="$t('uc.regist.country')" @on-change="onAreaChange">
                            <Option v-for="area in areas" :value="lang=='English'?area.enName:area.zhName" :key="lang=='English'? area.enName : area.zhName">{{lang=='English'?area.enName:area.zhName}}</Option>
                        </Select>
                    </FormItem>
                    <FormItem prop="user">
                    ...
                    <div style="color:#979797;display:inline-block;line-height:30px;font-size:12px;cursor:default">
                        ...
                        <a href="#/about-protocol" target="_blank" style="color:#D58C2A;">《{{$t('uc.regist.userprotocol')}}》</a>
                    </div>
                    <FormItem>
        ...
        data () {
            ...
            return {
                ...
                ruleInline: {
                    ...
                    country: [
                        { required: true, message: this.$t('uc.regist.countrytip'), trigger: 'change' }
                    ],
                    user: [
        ...
        methods: {
            updateLangData(){
                this.buttonLists = [
                {
                 ...
                 this.getAreas();
                this.ruleInline.username[0].message = this.$t('uc.regist.usernametip');
                this.ruleInline.password[0].message = this.$t('uc.regist.pwdtip');
               
                if(this.changeActive==0){
                    this.key=this.$t('common.phone');
                     this.ruleInline.code=[{ required: true, message:this.$t('uc.safe.codetip'), trigger: 'blur' }];
            ...
            getAreas(){
                this.$http.post(this.host + this.api.common.area).then(response => {
                    ...
                    this.formInline.country = this.lang=='English'?this.areas[212].enName:this.areas[212].zhName;
                    this.formInline.areaCode = this.areas[212].areaCode;
                });
            },
            actives(index){
